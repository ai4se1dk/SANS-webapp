...
def get_current_state() -> str:
    ...
    try:
        ...
        if hasattr(fitter, 'data') and fitter.data is not None:
            data = fitter.data
            lines.append(f"  Data: {len(data.x)} points, Q range [{data.x.min():.4f}, {data.x.max():.4f}]")
        else:
            lines.append("  Data: Not loaded")

        # Model info
        if hasattr(fitter, 'model') and fitter.model is not None:
            lines.append(f"  Model: {fitter.model.name if hasattr(fitter.model, 'name') else 'Unknown'}")
...
def enable_polydispersity(parameter_name: str, pd_type: str = "gaussian", pd_value: float = 0.1) -> str:
    """
...
def set_structure_factor(sf_name: str) -> str:
    ...
    """
    if not _check_tools_enabled():
        return "AI tools are disabled. Enable them in the sidebar to allow structure factor changes."
